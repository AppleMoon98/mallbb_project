...
import { getOne, register, remove } from "../../api/freeApi";
...
const ModifyComponent = ({id}) => {
    ...
    const handleClickModify = () => {
        ...
        formData.append("price", free.id)
        formData.append("title", free.title)
        formData.append("desc", free.desc)
        formData.append("delFlag", free.delFlag)

        for(let i=0; i<free.uploadFileNames; i++){
            formData.append("uploadFileNames", free.uploadFileNames[i])
        }
    ...
    const handleChangeFree = (e) => {
        free[e.target.name] = e.target.value

        setFree({...free})
    }

    const handleChangeFreeComplete = (e) => {
        const value = e.target.value
        free.delFlag = (value === 'Y')
        setFree({...free})
    }

    const deleteOIdImages = (imageName) => {
        
        const resultFileNames = free.uploadFileNames.filter(fileName => fileName !== imageName)

        free.uploadFileNames = resultFileNames
        ...
        <div className="border-2 border-sky-200 mt-2 m-2 p-4">
            ...
            <div className="flex justify-center">
                <div className="relative mb-4 flex w-full flex-wrap items-stretch">
                    ...
                    name="title" type={'text'} value={free.title} onChange={handleChangeFree}>{free.title}</input>
                    </div>
            </div>
             ...
             <div className="flex justify-center">
                <div className="relative mb-4 flex w-full flex-wrap items-stretch">
                    ...
                    name="content" rows="4" value={free.content} onChange={handleChangeFree}>{free.content}</textarea>
                    </div>
            </div>
            ...
            <div className="flex justify-center">
                <div className="relative mb-4 flex w-full flex-wrap items-stretch">
                    ...
                    name="startDate" type={'text'} value={free.createDate} onChange={handleChangeFree}>{free.createDate}</input>
                    </div>
            </div>       
            ...
            <div className="flex justify-center">
                <div className="relative mb-4 flex w-full flex-wrap items-stretch">
                    ...
                    <select name="delFlag" className="border-solid border-2 rounded m-1 p-2" onChange={handleChangeFree} value={free.delFlag}>
                ...
                <div className="relative mb-4 flex w-full flex-wrap items-stretch">
                    ...
                    <div className="w-4/5 justify-center flex flex-wrap items-start">
                        ...
                        <div className="flex justify-center flex-col w-1/3" key={i}>
                            ...
                            <img alt="img" src={`${prefix}/api/free/view/s_${imgFile}`}></img>
                        </div>
                    )}
                </div>
